...
const PHOTO_PLACEHOLDERS = [
  { label: 'Фото 1: Состояние до', color: 'from-gray-100 to-gray-200' },
  { label: 'Фото 2: Процесс работ', color: 'from-gray-50 to-gray-100' },
  { label: 'Фото 3: Завершение', color: 'from-gray-100 to-gray-200' },
]
    ...
    <div className="space-y-6">
      ...
      {session && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-xs text-gray-500">
          <span className="text-gray-700">{session.contract.title}</span> → {session.milestone.description}
          ...
          <div key={p} className="flex items-center gap-2">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
              ...
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                phase === p ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-400'
              }`}>{i + 1}</span>
        ...
        <div className="ml-auto">
          {phase !== 'results' && timeLeft > 0 && (
            <div className={`text-sm font-mono font-medium px-3 py-1.5 rounded-lg border ${
              timeLeft < 60 ? 'text-red-500 border-red-200 bg-red-50 animate-pulse' : 'text-gray-600 border-gray-200 bg-gray-50'
            }`}>
              {formatTime(timeLeft)}
            </div>
          )}
      ...
      {phase === 'commit' && (
        <div className="space-y-5">
          <div>
            <h3 className="text-gray-900 font-semibold mb-3">Фотоматериалы подрядчика</h3>
            <div className="grid grid-cols-3 gap-3">
              {PHOTO_PLACEHOLDERS.map((photo, i) => (
                <div key={i} className={`aspect-video rounded-xl bg-gradient-to-br ${photo.color} border border-gray-200 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-emerald-300 transition-colors group`}>
                  ...
                  <span className="text-xs text-gray-400 group-hover:text-gray-600 transition-colors">{photo.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-gray-900 font-semibold mb-3">Ваш голос</h3>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setVote('accept')} className={`p-5 rounded-xl border-2 transition-all flex flex-col items-center gap-3 ${
                vote === 'accept' ? 'border-emerald-400 bg-emerald-50 shadow-md' : 'border-gray-200 bg-white hover:border-emerald-300'
              }`}>
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${vote === 'accept' ? 'bg-emerald-500' : 'bg-gray-100'}`}>
                ...
                <div>
                  <div className={`font-semibold ${vote === 'accept' ? 'text-emerald-700' : 'text-gray-700'}`}>Принять</div>
                  <div className="text-xs text-gray-400">Работа выполнена</div>
                </div>
              </button>
              <button onClick={() => setVote('reject')} className={`p-5 rounded-xl border-2 transition-all flex flex-col items-center gap-3 ${
                vote === 'reject' ? 'border-red-400 bg-red-50 shadow-md' : 'border-gray-200 bg-white hover:border-red-300'
              }`}>
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${vote === 'reject' ? 'bg-red-500' : 'bg-gray-100'}`}>
                ...
                <div>
                  <div className={`font-semibold ${vote === 'reject' ? 'text-red-600' : 'text-gray-700'}`}>Отклонить</div>
                  <div className="text-xs text-gray-400">Требует доработки</div>
                </div>
              </button>
            </div>
          </div>
          ...
          <button onClick={handleCommit} disabled={!vote || loading} className={`w-full py-3.5 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${
            vote ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'
          }`}>
            {loading ? (
      ...
      {phase === 'reveal' && (
        <div className="space-y-5">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h3 className="text-gray-900 font-semibold mb-4">Ваш зафиксированный голос</h3>
            <div className="space-y-3">
              <div>
                <div className="text-xs text-gray-400 mb-1">Голос</div>
                <div className={`font-medium ${vote === 'accept' ? 'text-emerald-600' : 'text-red-600'}`}>
                  {vote === 'accept' ? 'ПРИНЯТЬ' : 'ОТКЛОНИТЬ'}
                </div>
              </div>
              <div>
                <div className="text-xs text-gray-400 mb-1">Хэш коммита (SHA-256)</div>
                <div className="font-mono text-xs text-gray-500 break-all bg-gray-50 p-2 rounded border border-gray-200">
                  {commitHash}
                </div>
              </div>
            </div>
        ...
        <div className="space-y-5">
          <div className="text-center py-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 ${
              session?.result === 'ACCEPT' ? 'bg-emerald-50' : session?.result === 'REJECT' ? 'bg-red-50' : 'bg-yellow-50'
            }`}>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h4 className="text-gray-500 text-sm mb-4">Распределение голосов (взвешенное)</h4>
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-sm mb-1.5">
                  <span className="text-emerald-600 font-medium">Принято</span>
                  <span className="text-gray-900 font-bold">{acceptWeight} / {totalWeight}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1.5">
                  <span className="text-red-500 font-medium">Отклонено</span>
                  <span className="text-gray-900 font-bold">{rejectWeight} / {totalWeight}</span>
                </div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
          ...
          {/* Individual votes */}
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h4 className="text-gray-500 text-sm mb-3">Голоса присяжных</h4>
              ...
              {session?.votes.map((v) => (
                <div key={v.id} className="flex items-center justify-between text-sm py-1.5 border-b border-gray-100 last:border-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${v.isExpert ? 'bg-purple-50 text-purple-600' : 'bg-gray-100 text-gray-500'}`}>
                      {v.isExpert ? 'Эксперт' : 'Гражданин'}
                    </span>
                    <span className="text-gray-500 font-mono text-xs">{v.citizen.walletAddress.slice(0, 12)}...</span>
                    <span className="text-gray-400 text-xs">x{v.weight}</span>
                  </div>
                  <span className={`font-medium ${
                    v.revealedVote === 'ACCEPT' ? 'text-emerald-600' : v.revealedVote === 'REJECT' ? 'text-red-500' : 'text-gray-400'
                  }`}>
