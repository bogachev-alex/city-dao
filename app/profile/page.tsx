...
const TIER_CONFIG = {
  Bronze: { color: 'text-orange-500', bg: 'from-orange-50 to-orange-100', border: 'border-orange-200', min: 0, max: 200 },
  Silver: { color: 'text-gray-500', bg: 'from-gray-100 to-gray-200', border: 'border-gray-300', min: 200, max: 500 },
  Gold: { color: 'text-yellow-600', bg: 'from-yellow-50 to-yellow-100', border: 'border-yellow-200', min: 500, max: 1000 },
}
    ...
    <div className="min-h-screen pt-16 bg-gray-50">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-2xl font-bold text-gray-900">Гражданин Алматы</h1>
                <span className={`px-3 py-1 rounded-full text-xs font-bold border bg-gradient-to-r ${tier.bg} ${tier.color} ${tier.border}`}>
                  {MOCK_PROFILE.tier}
                </span>
              </div>
              <div className="text-gray-500 text-sm font-mono mb-3">{MOCK_PROFILE.address}</div>
            ...
            <div className="sm:text-right">
              ...
              <div className="text-gray-500 text-sm">очков репутации</div>
              {nextTier && (
                <div className="mt-2">
                  <div className="text-xs text-gray-400 mb-1.5">До {nextTier}: {tier.max - MOCK_PROFILE.reputation} очков</div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Голосов подано', value: MOCK_PROFILE.stats.votesCast, color: 'text-blue-600', icon: '🗳️' },
            { label: 'Точность', value: `${MOCK_PROFILE.stats.accuracy}%`, color: 'text-emerald-600', icon: '🎯' },
            { label: 'Наград получено', value: `${MOCK_PROFILE.stats.rewardsEarned} USDC`, color: 'text-yellow-600', icon: '💰' },
            { label: 'Сессий жюри', value: MOCK_PROFILE.stats.juryAssignments, color: 'text-purple-600', icon: '⚖️' },
          ].map((stat) => (
        ...
        {activeTab === 'votes' && (
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="grid grid-cols-5 gap-4 px-5 py-3 border-b border-gray-200 text-xs text-gray-400 font-medium uppercase tracking-wider">
            ...
            {VOTING_HISTORY.map((entry, i) => (
              <div
                key={entry.id}
                className={`grid grid-cols-5 gap-4 px-5 py-4 items-center ${i < VOTING_HISTORY.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50 transition-colors`}
              >
                <div className="col-span-2">
                  <div className="text-gray-900 text-sm font-medium truncate">{entry.contract}</div>
                  <div className="text-gray-400 text-xs truncate">{entry.milestone}</div>
                ...
                <div>
                  <span className={`text-xs font-medium ${
                    entry.outcome === 'correct' ? 'text-emerald-600' : 'text-yellow-600'
                  }`}>
                    {entry.outcome === 'correct' ? '✓ Верно' : '~ Меньшинство'}
                  </span>
                </div>
                <div className={`text-right font-bold ${entry.repChange > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                  +{entry.repChange}
                </div>
              </div>
            ))}
        ...
        {activeTab === 'jury' && (
          <div className="space-y-3">
            {VOTING_HISTORY.map((entry) => (
              <div key={entry.id} className="bg-white border border-gray-200 rounded-xl p-5 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-gray-900 text-sm font-medium truncate">{entry.contract}</div>
                  <div className="text-gray-400 text-xs">{entry.milestone} • {entry.date}</div>
                </div>
                <div className={`text-sm font-bold ${entry.outcome === 'correct' ? 'text-emerald-600' : 'text-yellow-600'}`}>
                  +{entry.repChange} rep
                </div>
              </div>
            ))}
