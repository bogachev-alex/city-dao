  ...
  return (
    <div className="min-h-screen pt-16 bg-gray-50">
      ...
      <div className="bg-gradient-to-b from-white to-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <div className="text-xs text-emerald-600 uppercase tracking-wider mb-2">Гражданский краудфандинг</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-blue-50 border border-emerald-200 rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs text-gray-600">
            <div className="flex gap-2">
              <span className="shrink-0 w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs">1</span>
              <div><span className="font-medium text-gray-900">Создайте кампанию</span> — опишите проект и укажите бюджет</div>
            </div>
            <div className="flex gap-2">
              <span className="shrink-0 w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs">2</span>
              <div><span className="font-medium text-gray-900">Граждане скидываются</span> — от 500 ₸ до 500 000 ₸</div>
            </div>
            <div className="flex gap-2">
              <span className="shrink-0 w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs">3</span>
              <div><span className="font-medium text-gray-900">Государство добавляет</span> — субсидия по категории проекта</div>
            </div>
            <div className="flex gap-2">
              <span className="shrink-0 w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs">4</span>
              <div><span className="font-medium text-gray-900">Проект запускается</span> — через Amanat Protocol с жюри</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm text-gray-500">
            Найдено: <span className="text-gray-900 font-medium">{filtered.length}</span> кампаний
          </div>
          <Link
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <div className="text-gray-600 font-medium mb-2">Кампании не найдены</div>
            <div className="text-gray-400 text-sm">Попробуйте изменить фильтры или создайте новую кампанию</div>
          </div>
        )}

        {/* Matching ratios info */}
        <div className="mt-8 bg-white border border-gray-200 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Модели субсидирования</h3>
            ...
            {Object.entries(CATEGORY_CONFIG).filter(([key]) => key !== 'commercial').map(([key, cfg]) => (
              <div key={key} className="bg-gray-50 border border-gray-100 rounded-lg p-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-emerald-600 font-medium">{cfg.statePercent}%</span>
                </div>
